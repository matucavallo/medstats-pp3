...
@section('contenido')
    <div class="max-w-2xl mx-auto px-4 py-8">
        ...
        <form action="{{ route('servicios.update', $servicio) }}" method="POST" class="bg-white shadow rounded-lg border border-gray-200 p-6 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="nombre" class="block text-sm font-medium text-gray-700 mb-1">Nombre del Servicio</label>
                <input type="text" name="nombre" id="nombre" class="w-full border-2 border-gray-400 rounded-md shadow-sm px-4 py-2 focus:ring-2 focus:ring-gray-500" value="{{ old('nombre', $servicio->nombre) }}" required>
                @error('nombre')
            ...
            <div>
                ...
                <textarea name="descripcion" id="descripcion" rows="3" class="w-full border-2 border-gray-400 rounded-md shadow-sm px-4 py-2 focus:ring-2 focus:ring-gray-500">{{ old('descripcion', $servicio->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
